...
class Admin_model extends CI_Model
{
    ...
    public function simpan_barang($id_barang, $nama_barang, $stok, $satuan_id, $jenis_id, $image_name)
    {
        $data = array(
            'id_barang'       => $id_barang,
    ...
    public function get_detail_barang_keluar($id)
    {
        $this->db->select('*');
        $this->db->join('user u', 'bk.user_id = u.id_user');
        $this->db->join('barang b', 'bk.barang_id = b.id_barang');
        ...
        $this->db->order_by('id_barang_keluar', 'DESC');
        return $this->db->get('barang_keluar bk')->row_array();
    }
    ...
    public function getPersetujuanBarangMasuk(){
        return $this->db->where('status', '0')->order_by('tanggal_masuk','DESC')->limit(5)->get('barang_masuk')->result_array();
    }

    public function getPersetujuanBarangKeluar(){
        return $this->db->where('status', '0')->order_by('tanggal_keluar','DESC')->limit(5)->get('barang_keluar')->result_array();
    }
